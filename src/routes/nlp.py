...
async def index_project(request: Request, project_id: str, push_request: PushRequest):
    ...
    while has_records:
        ...
        is_inserted = nlp_controller.index_into_vectordb(
            project=project,
            chunks=page_chunks,
            do_reset=push_request.do_reset if is_first_batch else 0,
        ...
        if not is_inserted:
            ...
            status_code=status.HTTP_400_BAD_REQUEST,
            content={
                "message": ResponseSignal.INSERT_INTO_VECTORDB_ERROR.value
            })
...
async def search_index(request: Request, project_id: str, search_request: SearchRequest):
    ...
    if not results:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={
                "message": ResponseSignal.VECTORDB_SEARCH_ERROR.value
            })
    
    return JSONResponse(
        content={ 
            "message": ResponseSignal.VECTORDB_SEARCH_SUCCESS.value,
